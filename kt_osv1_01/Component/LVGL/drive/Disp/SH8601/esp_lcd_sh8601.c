...
esp_err_t sh8601_tx_param(void *panel, int lcd_cmd, const void *param, size_t param_size)
{
    esp_lcd_panel_t* flcdpanel = (esp_lcd_panel_t*)panel;
    sh8601_panel_t *sh8601 = __containerof(flcdpanel, sh8601_panel_t, base);
    esp_lcd_panel_io_handle_t io = sh8601->io;
    if (sh8601->flags.use_qspi_interface) {
        lcd_cmd &= 0xff;
        lcd_cmd <<= 8;
